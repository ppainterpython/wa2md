...
# Timestamp patterns
# ---------------------------------------------------------------------------
# Unicode character U+200E LEFT-TO-RIGHT MARK (LRM) may appear before media placeholders, e.g. "‎<Media omitted>" or "‎IMG-20200116-WA0001.jpg (file attached)" – handle this in regexes below.
_UNICODE_LRM = re.compile("\u200e")
_UNICODE_NNBSP = re.compile("\u202f")
...
_FILE_ATTACHED = re.compile(r"^.*<attached:\s+([a-zA-Z0-9_.-]+)>", re.IGNORECASE)
...
def _build_message(timestamp: datetime, 
...
def parse_text(text: str) -> list[Message]:
    ...
    for line in text.splitlines():
        line = _UNICODE_LRM.sub("", line)  # Remove any LTR marks that may appear before media placeholders
        line = _UNICODE_NNBSP.sub(" ", line)  # Replace narrow non-breaking spaces with regular spaces00
